...
- 分支 `main`；开始 Git HEAD `8ff71d5a9c41ac659ed1131d7b214e94fff48ce3`，开始时工作区干净且与 `origin/main` 一致。导入与验收完成时提交前 HEAD 仍为该值；最终发布提交 SHA 记录在本次交付回复中。
- 修改前完整测试为 99 passed。最终完整测试为 116 passed（11.71 秒）；`compileall` 通过，`pip check` 为 `No broken requirements found`。
- 全量提取批次 `20260805T083938Z-13d23b19`：76 新增、0 修改、16 未变化、0 缺失、0 文件级提取失败、3 配置排除。最终重审批次 `20260806T021714Z-a8d1e260`：0 新增、0 修改、92 未变化、0 缺失、3 配置排除；它复用既有 revision/source blocks，只核对 SHA-256，没有重新提取或 OCR。正式批次为 `20260806T021714Z-a8d1e260`。
- prepare 前正式版本一直是 `20260804T043436Z-5b70fd81`（16 个审核修订、2,020 个 source FTS 块、0 条 semantic）；prepare 未改变正式检索。最终 apply 后为 92 个审核修订、8,845 个顾问块、9,477 个 source FTS 切片、13,478 条正式 semantic。21 个 policy/style_case 文件哈希前后未变化；76 份新增资料只进入审核 source/semantic，没有自动提升为 policy。
...
- 块级处置（24,552 块）：顾问可用 8,845、仅风格/内部隔离 22、待核对 3,232、范围外/舍弃 11,303、无文字 1,131、提取失败 19；另有 4 个不合规营销主张块被质量阻断。顾问可检索字符 1,056,050，总提取字符 4,936,260。
- 主要舍弃：范围外 8,601 块；内部运营/目标/人员/排期/系统管理及仅内部物料约 2,689 块；真实联系方式或业务标识符 9 块；结果性营销风险 4 块。混合文档按块保留审核记录，未整份静默遗漏。
- 待核对重点：活动条件/时效 834 块；全国班或天津适用性 602 块；混合政策/内部复盘 446 块；日期级强时效报名规则 145 块；其余为课程体系、教学执行边界、师资和未归档链接。
...
| 领域 | 文件 | 块 | 正式语义记录 |
| --- | ---: | ---: | ---: |
| 课程产品与基本参数 | 79 | 8,847 | 982 |
| 班型定位与适合学员 | 78 | 1,909 | 1,424 |
| 学科、年级与课程内容 | 82 | 18,298 | 7,147 |
| 教材版本与配套练习 | 68 | 1,155 | 1,198 |
| 课程衔接与学习规划 | 80 | 1,641 | 796 |
| 师资、教学过程与学习服务 | 76 | 1,315 | 802 |
| 报名、缴费、续报、转班退费规则 | 55 | 2,948 | 1,129 |
| 活动营销、优惠与赠品 | 64 | 1,259 | 0 |

- 语义候选 39,814 条：批准并正式启用 13,478、待核对 6,460、舍弃 19,876；正式记录 source revision/block 绑定率 100%。关系记录包括 `continues_to` 296、`uses_textbook` 234、`taught_by` 252、`overlaps_with` 55、`includes_service` 52、`suitable_for` 47、`prerequisite_of` 11、`alternative_to` 1。
- 活动候选 1,259 条：有效 0、过期正式记录 0、待核对 834、冲突 0、舍弃 425。没有资料同时满足完整起止日期、适用条件、办理方式和权威来源，因此当前活动查询不返回事实；旧活动和续报话术不会冒充当前优惠。
...
- 图片对象 4,331 个，OCR 有文字 3,190 个；其中顾问可用图片/PDF OCR 块 1,947、待核对 903、舍弃 475、内部隔离 7、风险阻断 3。1,131 个图片对象无文字；19 个失败对象由 10 个无法解码的嵌入媒体和 9 个扫描 PDF 页组成，均有对象级失败状态，没有静默遗漏。16 份未变化存量未重复 OCR。
...
- 834 条活动候选需补齐权威来源、适用课程/新老生、起止日期、条件和办理方式；未补齐前保持不可检索。
