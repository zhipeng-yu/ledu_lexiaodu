...
- 普通聊天和直接提问使用自然回答；只有明确请求家长沟通建议或话术时才输出顾问/家长双段结构。
- 文档路由、知识检索和最终生成均以最新用户消息为本轮目标；历史按原始用户/助手角色传递，旧截图只挂在原消息上，不再伪装成本轮任务。
- 用户提出的输出要求在当前上下文持续生效；“不发链接、只发文字”另有结果清理兜底。
...
- `.\.venv\python.exe -m pytest -q`：112 项通过。
...
3. 每台设备各做最小真实普通对话、知识库回答和合成或脱敏截图理解；重点复验自然闲聊、连续切换问题不复读旧答案，以及“不发链接”在后续问题中持续有效。两台同时调用一次，核对统一账户扣费、数据隔离及是否出现并发或限流错误。
