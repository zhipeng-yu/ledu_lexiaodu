...
当前唯一入口是可重启恢复、严格按会话隔离的独立聊天窗口；聊天正文、OCR 校正文案、回复卡记录和附件均加密保存在本机。默认助手是明确标注的离线演示，不查询也不编造公司事实。旧悬浮工具条及其启动开关、一次性问答窗口和专属编排链路已经删除。
...
窗口支持新建、选择、重命名、搜索和删除本地会话，也支持“框选截图”和“粘贴截图”进入同一套 OCR 校正流程。发送消息时，离线演示助手会明确说明它不会查询或编造公司事实；生产文档顾问尚未接入这个入口，因此“生成正式回复”和“会话资料”当前禁用并显示说明，已恢复回复卡也不显示尚未接入存储的反馈控件。应用不再读取界面模式环境变量，也没有旧界面回滚入口。若只安装了 `.[dev]`，仍可使用 OCR 校正窗口的手动粘贴兜底；完整截图 OCR 演示需要安装 `.[dev,ocr]`。

点击“框选截图”后拖动鼠标选择一个聊天区域，或先把虚构截图复制到系统剪贴板再点击“粘贴截图”：

1. 聊天窗口把截图写成随机文件名的加密 `.bin` 附件；不会留下可直接打开的 PNG/JPG，也不会把原图交给助手。
...
所有窗口启动时默认使用“系统字号 + 1pt”；应用处于活动状态时可用 `Ctrl + +` / `Ctrl + -` 全局调节字号。调节仅在本次运行内有效，重启后恢复默认。OCR 识别完成后先停留在校正窗口；顾问确认的文字只进入截图开始时所属会话，原图不进入助手上下文。
...
- 截图原图只供本地加密保存和 OCR 使用，不会进入助手上下文；只有顾问确认后的 OCR 文字会进入其所属会话的上下文。
- 当前独立聊天使用离线助手，不会把 OCR 校正文案或会话上下文发送给方舟。后续接入原文档顾问时，参与回答的确认文字和上下文包将离开本机，正式放行前必须确认业务隐私与供应商数据处理要求。
...
- 独立聊天会恢复加密的会话、消息、OCR 校正文案和既有回复卡。
