...
当前默认入口是可重启恢复、严格按会话隔离的独立聊天窗口；聊天正文、OCR 校正文案、回复卡记录和附件均加密保存在本机。默认助手是明确标注的离线演示，不查询也不编造公司事实。原有截图、本地检索、豆包结构化建议、风险确认、编辑复制和匿名反馈流程仍保留在显式的旧版回滚模式中。
...
默认窗口支持新建、选择、重命名、搜索和删除本地会话，也支持“框选截图”和“粘贴截图”进入同一套 OCR 校正流程。发送消息时，离线演示助手会明确说明它不会查询或编造公司事实；生产文档顾问尚未接入这个入口，因此“生成正式回复”和“会话资料”当前禁用并显示说明，默认聊天中的已恢复回复卡也不显示尚未接入存储的反馈控件。要临时回滚到原置顶悬浮工具条，在同一 PowerShell 会话中显式设置：
...
只接受 `chat`（默认）或 `legacy`；其他值会在构造界面和后台运行对象前以配置错误码 `2` 退出。若只安装了 `.[dev]`，仍可使用 OCR 校正窗口的手动粘贴兜底；完整截图 OCR 演示需要安装 `.[dev,ocr]`。

点击“框选截图”后拖动鼠标选择一个聊天区域，或先把虚构截图复制到系统剪贴板再点击“粘贴截图”：

1. 默认聊天窗口把截图写成随机文件名的加密 `.bin` 附件；不会留下可直接打开的 PNG/JPG，也不会把原图交给助手。旧版回滚模式仍只在当前进程内保留截图。
...
在 `legacy` 回滚模式中，点击“AI 问答”可打开旧版手动问题分析窗口：
...
6. Enter 发送，Shift+Enter 换行；此处描述的是旧版窗口，退出后不持久化聊天正文。
...
- 默认截图原图只供本地加密保存和 OCR 使用，不会进入助手上下文；只有顾问确认后的 OCR 文字会进入其所属会话的上下文。旧版回滚模式仍不保存截图历史。
- 启用豆包后，OCR 校正对话和本次检索片段会发送给火山方舟生成建议；本机不落盘不等于数据不出本机，正式使用前需确认业务隐私与供应商数据处理要求。
- 旧版回滚模式的 `data/feedback.sqlite3` 仅保存建议 ID、有用状态、枚举原因和时间戳，表结构中没有聊天、问题或回复正文字段；默认聊天在生产反馈存储接入前不显示回复卡反馈控件。
...
- 默认独立聊天会恢复加密的会话、消息、OCR 校正文案和既有回复卡；旧版工具条中的临时聊天仍不恢复。
